...
def analyze_error_types():
    ...
    required_columns = [
        ...
        "FSSOCSEC",
        "FSSSI",
        "FSCSUPRT",  # Income sources
        "FSGRINC",
        "FSNETINC",
        "FSBEN",  # Income and benefits
        "FSSTDDED",
        "FSERNDED",
        "FSDEPDED",
        "FSMEDDED",
    ...
    for col, label in income_sources.items():
        if col in ms_df.columns:
            ...
            for name, subset in [
                ...
                print(
                    f"  {name:15s}: {has_income:3d} cases ({pct:5.1f}%) - Avg: ${avg_amt:.2f}"
                )
    ...
    for name, subset in [
        ...
        if "FSGRINC" in subset.columns:
            ...
            print(
                f"  Cases with $0 gross income: {zero_income} ({100*zero_income/len(subset):.1f}%)"
            )
    ...
    for col, label in deductions.items():
        if col in ms_df.columns:
            ...
            for name, subset in [
                ...
                avg_amt = (
                    subset[subset[col] > 0][col].mean() if has_deduction > 0 else 0
                )
                print(
                    f"  {name:15s}: {has_deduction:3d} cases ({pct:5.1f}%) - Avg: ${avg_amt:.2f}"
                )
    ...
    for name, subset in [
        ...
        if "FSNETINC" in subset.columns:
            ...
            print(
                f"  Cases with $0 net income: {zero_net} ({100*zero_net/len(subset):.1f}%)"
            )
    ...
    if all(col in ms_df.columns for col in ["FSGRINC", "FSTOTDED", "FSNETINC"]):
        ...
        if large_discrepancies.sum() > len(ms_df) * 0.1:  # More than 10% have issues
            print(
                f"WARNING: {large_discrepancies.sum()} cases have large net income discrepancies"
            )
            print(f"  This suggests data quality issues or complex deduction rules")

    # Create a comparison of gross → net → benefit
    if all(
        col in ms_df.columns for col in ["FSGRINC", "FSTOTDED", "FSNETINC", "FSBEN"]
    ):
        print("\nAverage Calculation Flow:")
        print(
            f"{'Case Type':<20} {'Gross Income':<15} {'Deductions':<15} {'Net Income':<15} {'Benefit':<15}"
        )
        print("-" * 80)
        ...
        for name, subset in [
            ...
            print(
                f"{name:<20} ${gross:<14.2f} ${deduct:<14.2f} ${net:<14.2f} ${benefit:<14.2f}"
            )
    ...
    for num_sources in sorted(ms_df["num_income_sources"].unique()):
        ...
        print(
            f"  {num_sources} income source(s): {error_rate:.1f}% error rate ({error_count}/{len(source_df)} cases)"
        )
    ...
    if "FSWAGES" in ms_df.columns:
        ...
        earned_error_rate = (
            100
            * len(earned_income_cases[earned_income_cases["STATUS"].isin([2, 3])])
            / len(earned_income_cases)
        )
        no_earned = ms_df[ms_df["FSWAGES"] == 0]
        no_earned_error_rate = (
            100 * len(no_earned[no_earned["STATUS"].isin([2, 3])]) / len(no_earned)
        )
        ...
        print(
            f"  Difference: {earned_error_rate - no_earned_error_rate:+.1f} percentage points"
        )
    ...
    if "FSMEDDED" in ms_df.columns:
        ...
        med_error_rate = (
            100 * len(with_med[with_med["STATUS"].isin([2, 3])]) / len(with_med)
        )
        no_med = ms_df[ms_df["FSMEDDED"] == 0]
        no_med_error_rate = (
            100 * len(no_med[no_med["STATUS"].isin([2, 3])]) / len(no_med)
        )
        ...
        print(
            f"  Difference: {med_error_rate - no_med_error_rate:+.1f} percentage points"
        )
    ...
    if "SHELDED" in ms_df.columns:
        ...
        shelter_error_rate = (
            100
            * len(with_shelter[with_shelter["STATUS"].isin([2, 3])])
            / len(with_shelter)
        )
        no_shelter = ms_df[ms_df["SHELDED"] == 0]
        no_shelter_error_rate = (
            100 * len(no_shelter[no_shelter["STATUS"].isin([2, 3])]) / len(no_shelter)
        )
        ...
        print(
            f"  Difference: {shelter_error_rate - no_shelter_error_rate:+.1f} percentage points"
        )
    ...
    if all(col in ms_df.columns for col in ["num_income_sources", "FSTOTDED"]):
        ...
        high_risk_error_rate = (
            100 * len(high_risk[high_risk["STATUS"].isin([2, 3])]) / len(high_risk)
            if len(high_risk) > 0
            else 0
        )
        ...
        low_risk_error_rate = (
            100 * len(low_risk[low_risk["STATUS"].isin([2, 3])]) / len(low_risk)
            if len(low_risk) > 0
            else 0
        )
