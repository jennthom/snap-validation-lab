...
def analyze_ms_snap_qc():
    ...
    if len(ms_df) < 100:
        print(
            f"WARNING: Only {len(ms_df)} MS cases found. Expected 1000+. Data may be incomplete."
        )
    ...
    assert (
        ms_df["STATUS"].isin([1, 2, 3]).all()
    ), "Invalid STATUS values: must be 1, 2, or 3"
    ...
    ), f"Status percentages don't sum to 100%: {total_pct:.1f}%"

    # Check poverty distribution sums to 100%
    poverty_total = sum(
        len(ms_df[(ms_df["TPOV"] >= min_val) & (ms_df["TPOV"] < max_val)])
        for min_val, max_val, _ in poverty_bins
    )
    poverty_pct = 100 * poverty_total / len(ms_df)
    assert (
        abs(poverty_pct - 100) < 1.0
    ), f"Poverty bins don't cover all cases: {poverty_pct:.1f}%"
    ...
    if "FSKID" in ms_df.columns:
        ...
        print(
            f"Households with children: {with_kids} ({100*with_kids/len(ms_df):.1f}%)"
        )
